...
int thermodynamics_at_z(
  ...
  /** - interpolate in table with array_interpolate_spline (normal mode) or array_interpolate_spline_growing_closeby (closeby mode) */
    ...
    if (((pth->reio_parametrization == reio_half_tanh) && (z < 2*pth->z_reio))
        ...
        || ((pth->reio_parametrization == reio_jwst_popIII) && (z < pth->reio_jwst_popIII_zpop + 10.))) {
...
int thermodynamics_free(
  ...
  if (pth->reio_parametrization == reio_jwst_popIII){
    free(pth->reio_jwst_z_table);
    free(pth->reio_jwst_x_table);
    free(pth->reio_jwst_ddx_table);
    free(pth->reio_jwst_popIII_Phi_table);
  }
...
int thermodynamics_indices(
  ...
  /* JWST pop-III neutral hydrogen fraction */
  class_define_index(pth->index_th_x_HI, pth->reio_parametrization == reio_jwst_popIII, index_th, 1);
  ...
  // != Martin
    /* case where x_e(z) is computed from the JWST UV-LF and Pop-III ODE */
  case reio_jwst_popIII:
    class_define_index(ptrp->index_re_xe_before,_TRUE_,index_re,1);
    class_define_index(ptrp->index_re_helium_fullreio_fraction,_TRUE_,index_re,1);
    class_define_index(ptrp->index_re_helium_fullreio_redshift,_TRUE_,index_re,1);
    class_define_index(ptrp->index_re_helium_fullreio_width,_TRUE_,index_re,1);
...
int thermodynamics_set_parameters_reionization(
  ...
  case reio_jwst_popIII:
    ...
    /* reionization starts when first ionizing sources appear (x_HII = 0 above this z) */
    preio->reionization_parameters[preio->index_re_reio_start] =
      fmin(pth->reio_jwst_popIII_zpop, ppr->reionization_z_start_max);

    preio->reionization_parameters[preio->index_re_helium_fullreio_fraction] =
      pth->YHe / (_not4_ * (1. - pth->YHe));

    class_test(pth->helium_fullreio_width == 0.,
               pth->error_message,
               "helium_fullreio_width = 0, stop to avoid division by zero");

    preio->reionization_parameters[preio->index_re_helium_fullreio_redshift] =
      pth->helium_fullreio_redshift;
    preio->reionization_parameters[preio->index_re_helium_fullreio_width] =
      pth->helium_fullreio_width;

    /* load UV-luminosity function table from file */
    class_call(thermodynamics_jwst_popIII_create_Phi_UV_table(pth, pth->error_message),
               pth->error_message, pth->error_message);

    /* solve ODE for x_HII(z) and store spline table */
    class_call(thermodynamics_jwst_popIII_calculate_x_H(ppr, pba, pth, pth->error_message),
               pth->error_message, pth->error_message);
    break;
...
int thermodynamics_calculate_remaining_quantities(
  ...
  /** - for reio_jwst_popIII: fill index_th_x_HI = 1 - x_HII from JWST ODE spline */
  if (pth->reio_parametrization == reio_jwst_popIII) {
    int index_z, jwst_last = 0;
    double xHII;
    for (index_z = 0; index_z < pth->tt_size; index_z++) {
      double z_row = pth->z_table[index_z];
      if (z_row >= pth->reio_jwst_z_table[0]) {
        /* above JWST integration range: use recombination x_e as proxy for x_HII */
        xHII = pth->thermodynamics_table[index_z * pth->th_size + pth->index_th_xe];
      }
      else if (z_row <= pth->reio_jwst_z_table[pth->reio_jwst_table_size - 1]) {
        xHII = pth->reio_jwst_x_table[pth->reio_jwst_table_size - 1];
      }
      else {
        class_call(array_interpolate_spline(
                     pth->reio_jwst_z_table,
                     pth->reio_jwst_table_size,
                     pth->reio_jwst_x_table,
                     pth->reio_jwst_ddx_table,
                     1, z_row, &jwst_last, &xHII, 1,
                     pth->error_message),
                   pth->error_message, pth->error_message);
      }
      pth->thermodynamics_table[index_z * pth->th_size + pth->index_th_x_HI] =
        fmax(0., fmin(1., 1. - xHII));
    }
  }
...
int thermodynamics_output_summary(
  ...
  case reio_jwst_popIII:
    printf(" -> JWST pop-III reionization gives optical depth = %f\n",pth->tau_reio);
    printf("    optical depth at zpop (z=%.2f) = %f\n",pth->reio_jwst_popIII_zpop,pth->jwst_popIII_tau);
    break;
...
int thermodynamics_reionization_get_tau(
  ...
  /** - --> for reio_jwst_popIII: also compute optical depth at zpop */
  if (pth->reio_parametrization == reio_jwst_popIII) {

    /* find first index where z_table[index_zpop] >= zpop
       (z_table is ascending: z_table[0]~0, z_table[tt_size-1]=z_max) */
    int index_zpop = 0;
    while (index_zpop < pth->tt_size - 1 &&
           pth->z_table[index_zpop] < pth->reio_jwst_popIII_zpop) {
      index_zpop++;
    }

    class_test(index_zpop < 3,
               pth->error_message,
               "zpop=%.2f too close to z=0, need at least 3 table points for spline",
               pth->reio_jwst_popIII_zpop);

    /* re-spline dkappa up to index_zpop (overwrites dddkappa scratch column) */
    class_call(array_spline_table_line_to_line(pth->tau_table,
                                               index_zpop,
                                               pth->thermodynamics_table,
                                               pth->th_size,
    ...
    pth->jwst_popIII_tau *= -1; /* same sign convention as tau_reio */
  }
 ...
 */
 ...
 int thermodynamics_reionization_function(
  ...
  // != Martin
  case reio_jwst_popIII: {
    /* interpolate precomputed x_HII(z) table built from the JWST ODE */
    int jwst_last = 0;
    double xHII;

    if (z >= pth->reio_jwst_z_table[0]) {
      /* above the integration range: neutral */
      xHII = 0.;
    }
    else if (z <= pth->reio_jwst_z_table[pth->reio_jwst_table_size - 1]) {
      /* below z=0 edge: use z=0 value */
      xHII = pth->reio_jwst_x_table[pth->reio_jwst_table_size - 1];
    }
    else {
      class_call(array_interpolate_spline(
                   ...
                   pth->reio_jwst_x_table,
                   pth->reio_jwst_ddx_table,
                   1, z, &jwst_last, &xHII, 1,
                   pth->error_message),
                 pth->error_message, pth->error_message);
    }

    xHII = fmax(0., fmin(1., xHII));

    /* f_e(z) step function following paper Eq.(2):
     * z < 4: He doubly ionized (He II->He III), f_e = 1 + Y/(2X) = 1 + 2*fHe
     * z >= 4: He singly ionized (tracks H reionization), f_e = 1 + Y/(4X) = 1 + fHe */
    double fHe = preio->reionization_parameters[preio->index_re_helium_fullreio_fraction];
    double fe = (z < 4.) ? (1. + 2.*fHe) : (1. + fHe);

    /* floor at the pre-reionization recombination residual so dkappa never vanishes */
    double xe_before = preio->reionization_parameters[preio->index_re_xe_before];
    *x = fmax(xe_before, fe * xHII);
    break;
  }
...
int thermodynamics_output_titles(
  ...
  class_store_columntitle(titles,"x_HI",pth->reio_parametrization == reio_jwst_popIII);
...
int thermodynamics_output_data(
  ...
  for (index_z=0; index_z<pth->tt_size; index_z++) {
    ...
    class_store_double(dataptr,pvecthermo[pth->index_th_x_HI],pth->reio_parametrization == reio_jwst_popIII,storeidx);
...
/**
 * Ionizing photon rate per hydrogen atom from Pop-III stars (paper Eq. 9):
 *
 *   ndot_ion / n_H = (1/n_H0) * int_{M_min}^{M_cut} dM_UV  Phi_UV * L_UV * xi_ion * f_esc
 *
 * The (1+z)^3 factors in ndot_ion and n_H cancel exactly.
 *
 * L_UV   : specific UV luminosity [erg/s/Hz],  log10(L_UV) = 0.4*(51.63 - M_UV)
 * xi_ion : ionizing efficiency   [Hz/erg],     log10(xi_ion) = a*M_UV + b + d*z
 *          (capped at M_UV = -16 and z = 9, paper Eq. 11)
 *
 * Integral evaluated by Gauss-Legendre quadrature over [M_min, M_cut].
 *
 * Returns: ndot_ion / n_H  [Mpc^{-1}], or 0 if z > z_star.
 */

// != Martin
int thermodynamics_jwst_popIII_ndot_ion_over_n_H(
    double z,
    double * ndot_ion_over_n_H,
    struct background * pba,
    struct thermodynamics * pth,
    ErrorMsg error_message
    ) {

  /** - return zero above the Pop-III onset redshift */
  if (z > pth->reio_jwst_popIII_zpop) {
    *ndot_ion_over_n_H = 0.;
    return _SUCCESS_;
  }

  /** - model parameters */
  double fesc   = pth->reio_jwst_popIII_fesc; /* escape fraction                          */
  double jwst_a = pth->reio_jwst_popIII_a;    /* xi_ion fit: log10(xi_ion)=a*M_UV+b+d*z  */
  double jwst_b = pth->reio_jwst_popIII_b;
  double jwst_d = pth->reio_jwst_popIII_d;

  /* UV magnitude integration range (notebook: -25 to 0) */
  double Mmin = -25.;
  double Mmax =   0.;

  /* third axis of Phi_UV table: log10(M_v^cut) scaled by sqrt(Omega_m h^2 / 0.143)
   * (0.143 is the reference Omega_m h^2 used when building the Phi_UV table) */
  double log10cutoff = pth->reio_jwst_popIII_logMvcut
                       + 0.5 * log10(pba->Omega0_m * pba->h * pba->h / 0.143);

  /* n_H0 in m^-3; convert to Mpc^-3 to match Phi_UV units [Mpc^-3 mag^-1] */
  /* _Mpc_over_m_ => gives  m/Mpc */
  double n_H0_Mpc3 = pth->n_e * pow(_Mpc_over_m_, 3.);

  /** - Gauss-Legendre quadrature over M_UV in [Mmin, Mmax] */
  int n_gl = 100;
  double *mu, *w8;
  class_alloc(mu, n_gl * sizeof(double), error_message);
  if ((w8 = malloc(n_gl * sizeof(double))) == NULL) {
    free(mu);
    class_stop(error_message, "Cannot allocate w8 array (%d doubles)", n_gl);
  }
  quadrature_gauss_legendre(mu, w8, n_gl, 1.e-10, error_message);

  double half_range = 0.5 * (Mmax - Mmin);
  double mid        = 0.5 * (Mmax + Mmin);
  double integral   = 0.;
  double M_UV, log10xi_ion, log10L_UV, log10Phi_UV_val, integrand;
  int status = _SUCCESS_;

  for (int i = 0; i < n_gl; i++) {

    M_UV = half_range * mu[i] + mid;

    /* xi_ion [Hz/erg]: capped at M_UV=-16 and z=9 (paper Eq. 11) */
    log10xi_ion = jwst_a * fmin(-16., M_UV) + jwst_b + jwst_d * fmin(9., z);

    /* L_UV [erg] (paper Eq. 10) */
    log10L_UV = 0.4 * (51.63 - M_UV);

    /* Phi_UV: table stores log10(Phi_UV * 1e-9) [Mpc^-3 mag^-1] */
    status = trilinear_interp(&pth->reio_jwst_popIII_Phi_grid,
                              M_UV, z, log10cutoff,
                              &log10Phi_UV_val,
                              error_message);
    if (status == _FAILURE_) break;

    /* integrand = Phi_UV * L_UV * xi_ion  [s^-1 Mpc^-3 mag^-1] */
    /* Phi_UV in units of 1/(kpc^3 * mag) -> converting to 1/(Mpc^3 * mag)*/
    integrand = pow(10., log10xi_ion + log10L_UV + 9. + log10Phi_UV_val);

    integral += half_range * w8[i] * integrand;
  }

  free(mu);
  free(w8);

  if (status == _FAILURE_) return _FAILURE_;

  // NB! Check units!!!!!
  /* [s^-1 Mpc^-3] * fesc / n_H0 [Mpc^-3] / c [Mpc/s] -> ndot_ion/n_H [Mpc^-1] */
  *ndot_ion_over_n_H = fesc * integral / n_H0_Mpc3 / _s_over_Mpc_;

  return _SUCCESS_;
}

// != Martin
int thermodynamics_jwst_popIII_create_Phi_UV_table(struct thermodynamics * pth,
                                                   ErrorMsg errmsg) {

  /* read n0, n1, n2 and axis bounds from file header */
  class_call(trilinear_read_header(pth->reio_jwst_popIII_Phi_UV_file,
                                   &pth->reio_jwst_popIII_Phi_UV_n0,
                                   &pth->reio_jwst_popIII_Phi_UV_n1,
                                   &pth->reio_jwst_popIII_Phi_UV_n2,
                                   &pth->reio_jwst_popIII_Phi_grid.x0_min,
                                   &pth->reio_jwst_popIII_Phi_grid.x1_min,
                                   &pth->reio_jwst_popIII_Phi_grid.x2_min,
                                   &pth->reio_jwst_popIII_Phi_grid.x0_max,
                                   &pth->reio_jwst_popIII_Phi_grid.x1_max,
                                   &pth->reio_jwst_popIII_Phi_grid.x2_max,
                                   errmsg),
             errmsg, errmsg);

  int n_total = pth->reio_jwst_popIII_Phi_UV_n0
               * pth->reio_jwst_popIII_Phi_UV_n1
               * pth->reio_jwst_popIII_Phi_UV_n2;

  /* allocate and fill table */
  class_alloc(pth->reio_jwst_popIII_Phi_table, n_total * sizeof(double), errmsg);

  class_call(trilinear_read_table(pth->reio_jwst_popIII_Phi_UV_file,
                                  n_total,
                                  pth->reio_jwst_popIII_Phi_table,
                                  errmsg),
             errmsg, errmsg);

  /* init interpolator */
  class_call(trilinear_init(&pth->reio_jwst_popIII_Phi_grid,
                            pth->reio_jwst_popIII_Phi_grid.x0_min,
                            pth->reio_jwst_popIII_Phi_grid.x0_max,
                            pth->reio_jwst_popIII_Phi_UV_n0,
                            pth->reio_jwst_popIII_Phi_grid.x1_min,
...
// != Martin
int thermodynamics_jwst_popIII_t_rec(double z,
                                      double * t_rec,
                                      struct background * pba,
                                      struct thermodynamics * pth,
                                      ErrorMsg errmsg
  ) {
  /* Case-B recombination coefficient at T = 2e4 K [Mpc^3 s^-1], eq (8) */
  // double alpha_B = 4.91e-87;
  double T_B = 2e4;
  // alpha_B = [Mpc^3/s] -> [Mpc^3 / Mpc]
  double alpha_B = 8.816 * 1e-87 * pow(T_B / 1e4, -0.845) / _s_over_Mpc_;
  double C = 3.;                   /* clumping factor */
  double x_He = pth->fHe;          /* nHe/nH */
  /* pth->n_e is in units of [m^-3] */
  /* Proper hydrogen number density today [Mpc^-3] */
  double n_H0_Mpc3 = pth->n_e * pow(_Mpc_over_m_, 3.);

  /* Proper n_H at redshift z [Mpc^-3] */
  double n_H = n_H0_Mpc3 * pow(1. + z, 3.);

  /* t_rec [Mpc], eq (8) */
  /* (Mpc^2 * Mpc^-3)^-1 = Mpc */
  *t_rec = 1. / (C * alpha_B * (1. + x_He) * n_H);
  return _SUCCESS_;
}

// != Martin
/**
 * Helper: evaluate dx_HII/dz at (z, xHII) for the reionization ODE.
 *
 * From eq (6) of the paper, converting dt → dz:
 *   dx_HII/dz = -(ndot_ion/n_H  -  xHII/t_rec) / ((1+z)*H(z))
 *
 * With H in Mpc^{-1}, ndot_ion/n_H in Mpc^{-1}, t_rec in Mpc.
 */
static int thermodynamics_jwst_popIII_dxHII_dz(
    double z, double xHII,
    struct precision * ppr,
    struct background * pba,
    struct thermodynamics * pth,
    double * pvecback, int * last_index,
    double * dxdz,
    ErrorMsg errmsg
  ) {

  double H_Mpc, ndot, t_rec_Mpc;

  /* background at z */
  class_call(background_at_z(pba, z, long_info, inter_normal, last_index, pvecback),
             pba->error_message, errmsg);

  /* H in Mpc^{-1} (CLASS native units) */
  H_Mpc = pvecback[pba->index_bg_H];

  /* ionizing photon rate per hydrogen atom [Mpc^{-1}] */
  class_call(thermodynamics_jwst_popIII_ndot_ion_over_n_H(z, &ndot, pba, pth, errmsg),
             errmsg, errmsg);

  /* recombination timescale [Mpc] */
  class_call(thermodynamics_jwst_popIII_t_rec(z, &t_rec_Mpc, pba, pth, errmsg),
             errmsg, errmsg);

  /* clamp to physical range for numerical stability */
  double x = fmax(0., fmin(1., xHII));

  /* dx_HII/dz = -(source - sink) / ((1+z)*H) */
  *dxdz = -(ndot - x / t_rec_Mpc) / ((1. + z) * H_Mpc);
...
// != Martin
/**
 * Solve the reionization ODE (eq 6 of the paper) for x_HII(z) and
 * store the result as a cubic spline in pth->reio_jwst_{z,x,ddx}_table.
 *
 * Integration uses RK4 from z_start (x_HII=0) down to z=0.
 * Tables are stored with decreasing z: z_table[0]=z_start, z_table[N-1]=0.
 *
 * @param ppr    Input: precision structure
 * @param pba    Input: background structure (needed for H(z))
 * @param pth    Input/Output: thermodynamics structure (tables filled here)
 * @param errmsg Output: error message
 */
int thermodynamics_jwst_popIII_calculate_x_H(
    struct precision * ppr,
    struct background * pba,
    struct thermodynamics * pth,
    ErrorMsg errmsg
  ...
  int iz;
  double z0, z1, h, xn, k1, k2, k3, k4;
  double * pvecback;
  int last_index = 0;

  /* integration range: from zpop (or reionization_z_start_max) down to 0 */
  double z_start = fmin(pth->reio_jwst_popIII_zpop, ppr->reionization_z_start_max);

  /* number of z grid points — match reionization sampling density */
  int Nz = (int)(z_start / ppr->reionization_sampling) + 2;
  pth->reio_jwst_table_size = Nz;

  /* allocate tables */
  class_alloc(pth->reio_jwst_z_table,    Nz * sizeof(double), errmsg);
  class_alloc(pth->reio_jwst_x_table,    Nz * sizeof(double), errmsg);
  class_alloc(pth->reio_jwst_ddx_table,  Nz * sizeof(double), errmsg);

  /* allocate background work vector */
  class_alloc(pvecback, pba->bg_size * sizeof(double), errmsg);

  /* uniform z grid in decreasing order: z_table[0]=z_start, z_table[Nz-1]=0 */
  double dz = z_start / (Nz - 1.);
  for (iz = 0; iz < Nz; iz++) {
    pth->reio_jwst_z_table[iz] = z_start - iz * dz;
  }
  pth->reio_jwst_z_table[Nz - 1] = 0.;   /* ensure exact zero */

  /* initial condition: neutral at z_start */
  pth->reio_jwst_x_table[0] = 2e-4;

  /* RK4: step from z_table[iz] to z_table[iz+1] (h < 0 since z decreasing) */
  int rk4_status = _SUCCESS_;
  for (iz = 0; iz < Nz - 1; iz++) {
    z0 = pth->reio_jwst_z_table[iz];
    z1 = pth->reio_jwst_z_table[iz + 1];
    h  = z1 - z0;                           /* negative step */
    xn = pth->reio_jwst_x_table[iz];

    if ((rk4_status = thermodynamics_jwst_popIII_dxHII_dz(z0,         xn,             ppr, pba, pth, pvecback, &last_index, &k1, errmsg)) == _FAILURE_) break;
    if ((rk4_status = thermodynamics_jwst_popIII_dxHII_dz(z0 + 0.5*h, xn + 0.5*h*k1, ppr, pba, pth, pvecback, &last_index, &k2, errmsg)) == _FAILURE_) break;
    if ((rk4_status = thermodynamics_jwst_popIII_dxHII_dz(z0 + 0.5*h, xn + 0.5*h*k2, ppr, pba, pth, pvecback, &last_index, &k3, errmsg)) == _FAILURE_) break;
    if ((rk4_status = thermodynamics_jwst_popIII_dxHII_dz(z1,          xn + h*k3,     ppr, pba, pth, pvecback, &last_index, &k4, errmsg)) == _FAILURE_) break;

    pth->reio_jwst_x_table[iz + 1] =
      fmax(0., fmin(1., xn + (h / 6.) * (k1 + 2.*k2 + 2.*k3 + k4)));
  }

  free(pvecback);
  if (rk4_status == _FAILURE_) return _FAILURE_;

  /* compute cubic spline second derivatives for later interpolation */
  class_call(array_spline_table_lines(pth->reio_jwst_z_table,
                                      Nz,
                                      pth->reio_jwst_x_table,
                                      1,
                                      pth->reio_jwst_ddx_table,
